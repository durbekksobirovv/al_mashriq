...
const FormInput = ({ label, icon: Icon, value, onChange, type="text" }) => (
  <div className="space-y-3 flex-1">
    ...
    <div className="relative">
      <Icon className="absolute left-6 top-1/2 -translate-y-1/2 text-red-500" size={18} />
      <input type={type} value={value} onChange={(e)=>onChange(e.target.value)} className="w-full bg-slate-50 p-6 pl-14 rounded-[2rem] font-bold outline-none border-2 border-transparent focus:border-red-500 transition-all shadow-inner" />
    </div>
  </div>
);
...
export const AddProductForm = ({ form, setForm, categories, onSubmit }) => (
  <div className="max-w-6xl mx-auto flex flex-col lg:flex-row gap-12 pb-20">
    <div className="flex-1 bg-white p-12 rounded-[4rem] shadow-xl border border-white">
      <h2 className="text-3xl font-black text-slate-900 italic mb-10">Yangi Taom <span className="text-red-500">Yaratish</span></h2>
      <div className="space-y-8">
        ...
        <button onClick={onSubmit} className="w-full bg-slate-900 text-white py-7 rounded-[2.5rem] font-black shadow-2xl hover:bg-red-600 transition-all active:scale-95 uppercase tracking-widest">TAOMNI QO'SHISH</button>
      </div>
    </div>
    <div className="w-full lg:w-[400px]">
      <div className="bg-white rounded-[4rem] overflow-hidden shadow-2xl border border-white sticky top-32 p-4">
        ...
        <div className="p-8">
          <p className="text-[10px] font-black text-red-500 uppercase mb-2">{form.category || "Bo'lim"}</p>

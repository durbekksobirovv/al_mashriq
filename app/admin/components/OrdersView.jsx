      ...
      <div className="w-full flex items-center justify-between bg-white border border-slate-200 p-6 rounded-[2rem] shadow-sm">
        <div className="flex items-center gap-5">
          <div className="p-4 bg-red-50 rounded-2xl border border-red-100">
            <PackageOpen size={28} className="text-red-500" />
          </div>
          <div>
            <h3 className="text-2xl font-black text-slate-800 tracking-tighter uppercase">
              Buyurtmalar <span className="text-red-500 italic">Navbati</span>
            </h3>
        ...
        <AnimatePresence mode="popLayout">
          {sortedOrders.map((order) => (
            <motion.div 
              ...
              className="bg-white p-5 md:p-8 rounded-[2.5rem] border border-slate-100 flex flex-col md:flex-row items-center justify-between gap-6 hover:border-red-200 transition-all duration-300 group shadow-sm"
            >
              {/* 📞 LEFT: Info */}
              <div className="flex items-center gap-6 min-w-[250px] w-full md:w-auto">
                <div className="w-16 h-16 rounded-[1.8rem] bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-red-500 group-hover:text-white transition-all duration-500 shadow-inner">
                  <Phone size={24} />
                </div>
                <div>
              ...
              <div className="flex-1 flex flex-wrap gap-2 w-full">
                {order.items?.map((it, i) => (
                  <span key={i} className="bg-slate-50/50 px-5 py-3 rounded-2xl text-[12px] font-black border border-slate-100 text-slate-600 flex items-center gap-3">
                    <span className="text-red-600 bg-red-50 px-2.5 py-1 rounded-lg border border-red-100 italic">
              ...
              <div className="w-full md:w-auto">
                <motion.button 
                  ...
                  className="w-full md:w-auto flex items-center justify-center gap-3 bg-red-500 hover:bg-red-600 text-white px-10 py-5 rounded-[1.8rem] text-sm font-black uppercase tracking-widest shadow-lg shadow-red-500/20 transition-all"
                >
